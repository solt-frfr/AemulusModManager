...
### How Mod Configuration Works

You can now include configuration options per mod. This allows users to choose various aspects of a mod to use. Two windows are used for this process, one for the modder, and one for the user. Essentially, it loads the folder as its own package, always being one higher than the mod its from.

#### User Window

By right clicking on a mod, a user can select what options they want to include when building the mod. It produces a json containing a list of a list of intergers (no, that wasn't a typo), stored in Config/{game}/Mods/{id}.json.

An example of a json generated by this window would look like this. This mod would have two options, with choice 1 being chosen for the first option, and choices 1, 3, and 4 being chosen for the second option.
...
#### Modder Window / config.json Format

A tool has been provided for modders to add configurations, also availible by right clicking the mod. This is stored inside the package folder as config.json. Though, if they wish, modders can write a json by hand. It produces a json like this, and is a template for modders who do not wish to use the provided tool. This config has two options.

```
[
  {
    "optionname": "The First Option",
    "optionnum": 2,
...
In order for configurations to work the folder format should look like this:
...
<Package Folder>
  /modconfig
    /config0
      /0
        /<Package Folder (for this config option)>
      /1
        /<Package Folder (for this config option)>
    /config1
      /0
...
No names are stored, you have to use numbers. It's not very complicated. The first option would be /config0, the second would be /config1, etc. with each of the choices being a number starting from 0.
